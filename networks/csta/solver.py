...
class Solver(object):
    ...
    def build(self):
        ...
        if self.config.model == 'PGL_SUM':
            self.model = PGL_SUM(input_dim=self.config.input_dim, feature_size=1024, output_size=1024, num_segments=4, heads=8, fusion="add", pos_enc="absolute")
        ...
        self.writer = SummaryWriter(log_dir=f'{self.config.tensorboard_path}/{self.config.tag}')
    ...
    def log_cpu_ram_usage(self, step):
        ram_usage = psutil.virtual_memory().used / 1e9  # GB 단위
        total_ram = psutil.virtual_memory().total / 1e9  # GB 단위
        self.writer.add_scalar("CPU/RAM_Used_GB", ram_usage, step)
        self.writer.add_scalar("CPU/RAM_Total_GB", total_ram, step)
    ...
    def train(self):
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for batch_idx in tqdm(range(num_batches)):
                ...
                text_feature = data['text_feature'].to(self.config.device)
                audio_feature = data['audio_feature'].to(self.config.device)
                ...
                if self.config.model == 'CSTA':
                    # print(f"vis feature in Solver: {vis_feature.shape}", flush=True)  #torch.Size([batch_size, frame_num, emb_dim])
                    # print(f"text feature in Solver: {text_feature.shape}", flush=True)   #torch.Size([batch_size, frame_num, emb_dim])
                    # print(f"audio feature in Solver: {audio_feature.shape}", flush=True)   #torch.Size([batch_size, frame_num, emb_dim])
                    # print(f"mask in Solver: {mask.shape}", flush=True)  #torch.Size([batch_size, frame_num])
                    # print(f"gtscore: {gtscore.shape}, {gtscore.dtype}", flush=True) #torch.Size([batch_size, frame_num])
                    
                    score = self.model(vis_feature, text_feature, audio_feature, mask, self.config.mix_type)
                    ...
                    score, weights = self.model(vis_feature, text_feature, audio_feature, mask, self.config.mix_type)
    ...
    def evaluate(self, dataloader=None, test=False):
        ...
        for data in dataloader:
            ...
            #print(f"vis_feature in Solver: {vis_feature.shape}", flush=True) #torch.Size([batch_size=1, frame_num, emb_dim])
            #print(f"gtscore in Solver: {gtscore.shape}", flush=True) #torch.Size([batch_size, frame_num])
            ...
            if 'mask' in data:
                ...
                #print(f"mask in Solver: {mask.shape}", flush=True) # torch.Size([batch_size=1, frame_num])

            with torch.no_grad():
                if self.config.model == 'CSTA':
                    score = self.model(vis_feature, text_feature, audio_feature, mask, self.config.mix_type).detach()
                    score = score.squeeze()[:int(data['n_frames'][0])]
                    gtscore = gtscore.squeeze()[:int(data['n_frames'][0])]
                else:
                    score, attn_weights = self.model(vis_feature, text_feature, audio_feature, mask, self.config.mix_type)
                    score = score.squeeze()[:int(data['n_frames'][0])]
                    gtscore = gtscore.squeeze()[:int(data['n_frames'][0])]

...
class FixedPositionalEncoding(nn.Module):
    ...
    def forward(self, x, mask):
        pe = self.pe[:x.shape[1]].unsqueeze(0).expand(x.shape[0], -1, -1)   
        #print(f"mask in FixedPositionalEncoding before reshape: {mask.shape}", flush=True)
        mask = mask.squeeze().unsqueeze(-1)
        #print(f"mask in FixedPositionalEncoding after reshape: {mask.shape}", flush=True)

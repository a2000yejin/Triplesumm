...
from networks.a2summ.helpers.bbox_helper import offset2bbox
...
class Model_BLiSS(nn.Module):
    def __init__(self, config=None):
        super().__init__()
        input_dim_vis = config.input_dim_vis
        ...
        self.proj_fc_video = nn.Sequential(
                                nn.Linear(input_dim_vis, num_hidden, bias=True),
                                nn.Dropout(config.dropout_video),
                            )
        self.proj_fc_text = nn.Sequential(
                                nn.Linear(input_dim_text, num_hidden, bias=True),
        ...
        self.multiway_list = nn.ModuleList([MultiWayTransformer(num_hidden, dropout_attn=config.dropout_attn)] * config.num_layers)
    ...
    def forward(self, **kwargs):
        ...
        B, N_text, C = text.shape #N_video = N_text
        N_unique_sent_list = kwargs['n_sents']
        text_segment_mask_list = kwargs['text_segment_mask']
        print(f"N_unique_sent_list: {N_unique_sent_list}", flush=True)
        print(f"text_segment_mask_list: {text_segment_mask_list}", flush=True)

        video = video + self.pos_embed_video[:, :N_video, :] + self.type_video
        
        print(f"text: {text.shape}, N_unique_sents: {N_unique_sent}", flush=True)
        print(f"self.pos_embed_text: {self.pos_embed_text.shape}", flush=True)
        print(f"self.pos_embed_text[:, :N_text, :]: {self.pos_embed_text[:, :N_text, :].shape}", flush=True)
        print(f"self.type_text: {self.type_text.shape}", flush=True)
        print(f"self.pos_embed_segment: {self.pos_embed_segment.shape}", flush=True)
        pos_embed_segment = self.pos_embed_segment[:, :N_unique_sent, :]  # [batch_size, N_unique_sent, emb_dim]
...
class Model_VideoSumm(nn.Module):
    def __init__(self, args=None):
        super().__init__()
        input_dim_vis = args.input_dim_vis
